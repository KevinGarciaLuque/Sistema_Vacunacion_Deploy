...
export async function generarCarnetSimplePDF({ historial, usuario, dni }) {
  ...
  try {
    ...
    // --- AJUSTE PARA GARANTIZAR SELLO EN LA PRIMERA PÁGINA ---
    let firmaY = doc.lastAutoTable.finalY - 30;
    let espacioDisponible = pageHeight - firmaY - margenInferior;

    if (firmaY + selloHeight + 50 > pageHeight) {
      selloHeight = 180;
      selloWidth = 180;
      firmaY = pageHeight - selloHeight - 50; //evita que suba demasiado y choque con la tabla
    }
